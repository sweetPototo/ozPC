...
import javax.swing.WindowConstants;
	...
	public class join1 extends JFrame /*implements ViewPro*/ {
		LoginFrame owner;
		Userinfo users;
		UserDTO user;
		memdao dao; 
		...
		//정보 하나라도 비어있으면
	    if(Blank()) {
	        JOptionPane.showMessageDialog(join1.this, "빈칸없이 입력해주세요." );
	        return;
	        } 
	       
	        // 비밀번호와 비번확인이 일치하지 않았을 때
	    if(!String.valueOf(pwd1.getPassword()).equals(String.valueOf(pwd2.getPassword()))) {
	        		JOptionPane.showMessageDialog(join1.this, "비밀번호와 비밀번호확인이 일치하지 않습니다." );
	        		pwd1.requestFocus();
	   }else {
	            
	            user = new UserDTO(id.getText(), String.valueOf(pwd1.getPassword()), String.valueOf(pwd2.getPassword()),
	            		name.getText(), birth.getText(), phone.getText(), email.getText());

	    int result = dao.userInsert(user.getId(), user.getPwd(), user.getName(), 
